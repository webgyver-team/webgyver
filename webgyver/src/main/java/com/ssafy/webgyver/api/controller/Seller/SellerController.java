...
public class SellerController {
    ...
    @PostMapping("/signup")
    public ResponseEntity<MemberSignUpPostRes> signUp(@RequestBody SellerSignUpPostReq signUpInfo) {
        //임의로 리턴된 User 인스턴스. 현재 코드는 회원 가입 성공 여부만 판단하기 때문에 굳이 Insert 된 유저 정보를 응답하지 않음.
        System.out.println("Controller 들어옴");
        System.out.println(signUpInfo.toString());
        System.out.println("sellerService 결과 : "+sellerService.SignUpSeller(signUpInfo).getCategory());
